...
class ECB(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, depth_multiplier, act_layer=nn.GELU):
        ...
        self.conv3x3 = torch.nn.Conv2d(self.in_dim, self.out_dim, kernel_size=3, padding=1)
        self.conv1x1_3x3 = SeqConv3x3('conv1x1-conv3x3', self.in_dim, self.out_dim, self.depth_multiplier)
        self.conv1x1_sbx = SeqConv3x3('conv1x1-sobelx', self.in_dim, self.out_dim)
        self.conv1x1_sby = SeqConv3x3('conv1x1-sobely', self.in_dim, self.out_dim)
        self.conv1x1_lpl = SeqConv3x3('conv1x1-laplacian', self.in_dim, self.out_dim)

        self.act = act_layer()

    def forward(self, x):
        if self.training:
            x = self.conv3x3(x) + self.conv1x1_3x3(x) + self.conv1x1_sbx(x) + self.conv1x1_sby(x) + self.conv1x1_lpl(x) + x
        else:
            rep_weight, rep_bias = self.rep_params()
            x = F.conv2d(input=x, weight=rep_weight, bias=rep_bias, stride=1, padding=1)
    ...
    def rep_params(self):
        ...
        if device < 0:
            device = None
        weight_idt = torch.zeros(self.out_dim, self.out_dim, 3, 3, device=device)
...
class ECB_eval(nn.Module):
    def __init__(self, in_dim, out_dim, depth_multiplier, act_layer=nn.GELU):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.conv = torch.nn.Conv2d(self.in_dim, self.out_dim, kernel_size=3, padding=1)
        self.act = act_layer()
...
class TokenMixerCNN(nn.Module):
    def __init__(self, dim, kernel_size, depth_multiplier=2, mode='eval'):
        super().__init__()
        if mode == 'eval':
            self.ecb = ECB_eval(in_dim=dim, out_dim=dim, depth_multiplier=2, act_layer=nn.GELU)
        elif mode == 'train':
            self.ecb = ECB(in_dim=dim, out_dim=dim, depth_multiplier=depth_multiplier, act_layer=nn.GELU)
...
class TokenMixer(nn.Module):
    def __init__(self, dim, window_size, num_heads, kernel_size=5, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., mode='eval', **kwargs):
        super().__init__()
        self.dim = dim
        ...
        # Adding CNN layer
        self.cnn = TokenMixerCNN(dim=dim//2, kernel_size=kernel_size, mode=mode)
