...
class FourierUnit(nn.Module):
    def __init__(self, embed_dim, fft_norm='ortho'):
        ...
        self.conv_layer = nn.Conv2d(embed_dim * 2, embed_dim * 2, 1, 1, 0, groups=embed_dim * 2)
...
class SpectralTransform(nn.Module):
    def __init__(self, embed_dim, last_conv=False):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim // 2, 1, 1, 0, groups=embed_dim//2),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
        self.fu = FourierUnit(embed_dim // 2)

        self.conv2 = nn.Conv2d(embed_dim // 2, embed_dim, 1, 1, 0, groups=embed_dim//2)

        if self.last_conv:
            self.last_conv = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1, groups=embed_dim)
...
class SFB(nn.Module):
    def __init__(self, embed_dim, red=1):
        ...
        self.fusion = nn.Conv2d(embed_dim * 2, embed_dim, 1, 1, 0, groups=embed_dim)
